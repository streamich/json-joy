...
export const compactToOp = (op: CompactOp): Op => {
  switch (op[0]) {
    case OPCODE.add: return new OpAdd(toPath(op[1]), op[2]);
    case OPCODE.remove: return new OpRemove(toPath(op[1]), op[2]);
    case OPCODE.replace: return new OpReplace(toPath(op[1]), op[2], op[3]);
    case OPCODE.move: return new OpMove(toPath(op[1]), toPath(op[2]));
    case OPCODE.copy: return new OpCopy(toPath(op[1]), toPath(op[2]));
    ...
    default: return compactToPredicateOp(op);
  }
};

export const compactToPredicateOp = (op: CompactOp): PredicateOp => {
  switch (op[0]) {
    case OPCODE.test: return new OpTest(toPath(op[1]), op[2], !!op[3]);
    case OPCODE.defined: return new OpDefined(toPath(op[1]));
    case OPCODE.undefined: return new OpUndefined(toPath(op[1]));
    case OPCODE.type: return new OpType(toPath(op[1]), op[2]);
    case OPCODE.test_type: return new OpTestType(toPath(op[1]), op[2]);
    case OPCODE.test_string: return new OpTestString(toPath(op[1]), op[2], op[3], !!op[4]);
    case OPCODE.test_string_len: return new OpTestStringLen(toPath(op[1]), op[2], !!op[3]);
    case OPCODE.contains: return new OpContains(toPath(op[1]), op[2], !!op[3]);
    case OPCODE.ends: return new OpEnds(toPath(op[1]), op[2], !!op[3]);
    case OPCODE.starts: return new OpStarts(toPath(op[1]), op[2], !!op[3]);
    case OPCODE.matches: return new OpMatches(toPath(op[1]), op[2], !!op[3]);
    case OPCODE.in: return new OpIn(toPath(op[1]), op[2]);
    case OPCODE.less: return new OpLess(toPath(op[1]), op[2]);
    case OPCODE.more: return new OpMore(toPath(op[1]), op[2]);
    case OPCODE.and: {
