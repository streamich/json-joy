...
test('passes through websocket ready state', () => {
  ...
  expect(rx.state()).toBe(ws!.readyState);
  expect(rx.state()).toBe(WebSocketState.CONNECTING);
  expect(rx.isOpen()).toBe(false);
  expect(rx.isConnecting()).toBe(true);
  expect(rx.isClosing()).toBe(false);
  expect(rx.isClosed()).toBe(false);

  ws!._readyState = WebSocketState.OPEN;

  expect(rx.state()).toBe(ws!.readyState);
  expect(rx.state()).toBe(WebSocketState.OPEN);
  expect(rx.isOpen()).toBe(true);
  expect(rx.isConnecting()).toBe(false);
  expect(rx.isClosing()).toBe(false);
  expect(rx.isClosed()).toBe(false);

  ws!._readyState = WebSocketState.CLOSING;

  expect(rx.state()).toBe(ws!.readyState);
  expect(rx.state()).toBe(WebSocketState.CLOSING);
  expect(rx.isOpen()).toBe(false);
  expect(rx.isConnecting()).toBe(false);
  expect(rx.isClosing()).toBe(true);
  expect(rx.isClosed()).toBe(false);

  ws!._readyState = WebSocketState.CLOSED;

  expect(rx.state()).toBe(ws!.readyState);
  expect(rx.state()).toBe(WebSocketState.CLOSED);
  expect(rx.isOpen()).toBe(false);
  expect(rx.isConnecting()).toBe(false);
  expect(rx.isClosing()).toBe(false);
  expect(rx.isClosed()).toBe(true);  
});
...
test.todo('when constructor crashes');
...
describe('.message$', () => {
  ...
  test('emits when websocket receives a message', async () => {
    ...
    ws._message(uint8.buffer);
    expect(message).toHaveBeenCalledTimes(3);
    expect(message).toHaveBeenCalledWith(uint8.buffer);
  });
});
