    ...
    if (isObservable(data)) {
      ...
      subscribeCompleteObserver<unknown>(req$, {
        next: (value) => {
          ...
          const message = new msg.RequestDataMessage(id, messageMethod, new AnyValue(value));
          this.buffer.push(message);
        },
        error: (error) => {
          cleanup();
          const messageMethod = firstMessageSent ? '' : method;
          const message = new msg.RequestErrorMessage(id, messageMethod, new AnyValue(error));
          this.buffer.push(message);
        },
        complete: (value) => {
          cleanup();
          const messageMethod = firstMessageSent ? '' : method;
          const message = new msg.RequestCompleteMessage(id, messageMethod, new AnyValue(value));
          this.buffer.push(message);
        },
      });
      data.subscribe(req$);
    } else {
      this.buffer.push(new msg.RequestCompleteMessage(id, method, new AnyValue(data)));
      req$.complete();
      cleanup();
    }
