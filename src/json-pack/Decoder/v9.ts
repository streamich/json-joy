...
export class Decoder {
  ...
  /** @ignore */
  protected primitiveValue(): unknown {
    const byte = this.u8();
    if (byte >= 0xe0) return byte - 0x100; // 0xe0
    if (byte <= 0xbf) {
      if (byte < 0x90) {
  ...
  public valSkip(): number {
    ...
    if (byte <= 0xbf) {
      if (byte < 0x90) {
        ...
        return this.objSkip(byte & 0b1111); // 0x80
      } else {
        if (byte < 0xa0) return this.arrSkip(byte & 0b1111);
        // 0x90
        else return this.str(byte & 0b11111); // 0xa0
      }
    }
  ...
  protected objToJsonPackValue(size: number): JsonPackValue {
    const obj: Record<string, unknown> = {};
    for (let i = 0; i < size; i++) {
      const key = this.key();
      obj[key] = this.val();
  ...
  protected strSkip(size: number): string {
    const uint8 = this.uint8;
    const end = this.x + size;
    let x = this.x;
    let str = '';
