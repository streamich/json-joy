...
import {decodeCompleteMessages} from '../decode';
...
test('decodes a simple un-subscribe message', () => {
  ...
  const messages2 = decodeCompleteMessages(buf, 0);
...
test('decodes long messages - 2', () => {
  const messages1 = [
    new UnsubscribeMessage(0xAAAA),
    new SubscribeMessage(4, 'asdfasdfasdlfkajs0923lskjdfasdf', new Uint8Array('1'.repeat(0b1_0000000_0000).split('').map(Number))),
    new UnsubscribeMessage(0xAAAA),
  ];
  const buf = encoder.encode(messages1);
  const messages2 = decodeCompleteMessages(buf, 0);
...
test('decodes long messages - 3', () => {
  const messages1 = [
    new UnsubscribeMessage(0xAAAA),
    new SubscribeMessage(0x1234, 'google.who', new Uint8Array('1'.repeat(0b1_1111111_0000000_0000).split('').map(Number))),
    new UnsubscribeMessage(0xAAAA),
  ];
  const buf = encoder.encode(messages1);
  const messages2 = decodeCompleteMessages(buf, 0);
