...
describe('clone()', () => {
  test('can clone a simple document', () => {
    ...
    expect(doc1.toView()).toEqual({foo: 'bar', gg: [123]});
    expect(doc2.toView()).toEqual({foo: 'bar', gg: [123]});
  ...
  test('can modify the cloned copy independently', () => {
    ...
    expect(doc1.toView()).toEqual({foo: 'bar', hh: true, hmmm: 'aha'});
    expect(doc2.toView()).toEqual({foo: 'bar', hh: true, lala: {gaga: 'land'}});
  });
});
...
describe('fork()', () => {
  test('can fork a simple document', () => {
    ...
    expect(doc1.toView()).toEqual([1, 2, 'lol']);
    expect(doc2.toView()).toEqual([1, 2, 'lol']);
  });
  ...
  test('can modify the cloned copy independently', () => {
    ...
    doc1.applyPatch(builder3.patch);
    expect(doc1.toView()).toEqual([false, 1, 2, 'lol']);
    expect(doc2.toView()).toEqual([true, 1, 2, 'lol']);
  });
});
