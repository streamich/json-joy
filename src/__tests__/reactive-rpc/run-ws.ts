...
const serverScript = 'demo:reactive-rpc:server:ws';
const testSuiteScript = 'test:reactive-rpc:jest:ws';

const startServer = async () => {
  const started = new Defer<void>();
  const exitCode = new Defer<number>();
  const cp = spawn('yarn', [serverScript], {
    shell: true,
  });
...
const runTests = async () => {
  const exitCode = new Defer<number>();
  const cp = spawn('yarn', [testSuiteScript], {
    env: {
      ...process.env,
      TEST_E2E: '1',
    },
