...
export class JsonPathCodegen {
  ...
  protected generateSelector(selector: types.IAnySelector, inputVar: string, outputVar: string): void {
    switch (selector.type) {
      ...
      case 'recursive-descent':
        this.genRecursiveDescentSelector(selector, inputVar, outputVar);
        break;
    }
  }
  ...
  protected generateIndexSelector(
    ...
    codegen.if(`Array.isArray(${dataVar})`, () => {
      ...
      codegen.if(`${indexVar} < 0`, () => {
        codegen.js(`${indexVar} = ${dataVar}.length + ${indexVar};`);
      });
      codegen.if(`${indexVar} >= 0 && ${indexVar} < ${dataVar}.length`, () => {
        codegen.js(`var child = ${dataVar}[${indexVar}];`);
        codegen.if(`child !== undefined`, () => {
  ...
  protected generateSliceSelector(
    ...
    if (step === 0) return;
    const iVar = codegen.var();
    const inputNodeVar = codegen.var();
    const dataVar = codegen.var();
    const lenVar = codegen.var();
    const startVar = codegen.var();
    const endVar = codegen.var();
    const nStartVar = codegen.var();
    const nEndVar = codegen.var();
    const lowerVar = codegen.var();
    const upperVar = codegen.var();
    const idxVar = codegen.var();
    const len = `${inputVar}.length`;
    codegen.js(`for (${iVar} = 0, ${inputNodeVar}, ${dataVar}; ${iVar} < ${len}; ${iVar}++) {`);
    codegen.js(`${inputNodeVar} = ${inputVar}[${iVar}];`);
    codegen.js(`${dataVar} = ${inputNodeVar}.data;`);
    codegen.if(`Array.isArray(${dataVar})`, () => {
      codegen.js(`${lenVar} = ${dataVar}.length;`);

      // Determine default start and end
      if (step >= 0) {
        codegen.js(`${startVar} = ${selector.start !== undefined ? selector.start : 0};`);
        codegen.js(`${endVar} = ${selector.end !== undefined ? selector.end : lenVar};`);
      } else {
        codegen.js(`${startVar} = ${selector.start !== undefined ? selector.start : `${lenVar} - 1`};`);
        codegen.js(`${endVar} = ${selector.end !== undefined ? selector.end : `-${lenVar} - 1`};`);
      }

      // Normalize indices
      codegen.js(`${nStartVar} = ${startVar} >= 0 ? ${startVar} : ${lenVar} + ${startVar};`);
      codegen.js(`${nEndVar} = ${endVar} >= 0 ? ${endVar} : ${lenVar} + ${endVar};`);
  ...
  protected generateFilterSelector(
    ...
    const elementValueVar = codegen.r();
    const keyVar = codegen.r();
    const memberValueVar = codegen.r();
    const memberValueNodeVar = codegen.r();
    const lenVar = codegen.var(`${inputVar}.length`);

    codegen.js(`for (var ${iVar} = 0, ${inputNodeVar}, ${dataVar}; ${iVar} < ${lenVar}; ${iVar}++) {`);
    codegen.js(`${inputNodeVar} = ${inputVar}[${iVar}];`);
    codegen.js(`${dataVar} = ${inputNodeVar}.data;`);
  ...
  protected generateCompareEqual(leftVar: string, rightVar: string): string {
    ...
    codegen.if(`left === undefined && right === undefined`, () => {
      codegen.js(`return true;`);
    });
    codegen.if(`left === undefined || right === undefined`, () => {
      codegen.js(`return false;`);
    });
    codegen.if(`Array.isArray(left) && left.length === 1`, () => {
      codegen.js(`left = left[0];`);
    });
    codegen.if(`Array.isArray(right) && right.length === 1`, () => {
      codegen.js(`right = right[0];`);
    });
    codegen.if(`left === undefined || right === undefined`, () => {
      codegen.js(`return false;`);
    });
    codegen.if(`left === right`, () => {
      codegen.js(`return true;`);
    });
    codegen.if(`typeof left !== typeof right`, () => {
      codegen.js(`return false;`);
    });
    codegen.if(`Array.isArray(left) && Array.isArray(right)`, () => {
      codegen.if(`left.length !== right.length`, () => {
        codegen.js(`return false;`);
      });
      codegen.js(`for (var i = 0; i < left.length; i++) {`);
      codegen.if(`!${fnName}(left[i], right[i])`, () => {
        codegen.js(`return false;`);
      });
      codegen.js(`}`);
      codegen.js(`return true;`);
    });
    codegen.if(`left && right && typeof left === "object" && typeof right === "object"`, () => {
      codegen.js(`var leftKeys = Object.keys(left);`);
      codegen.js(`var rightKeys = Object.keys(right);`);
      codegen.if(`leftKeys.length !== rightKeys.length`, () => {
        codegen.js(`return false;`);
      });
      codegen.js(`for (var i = 0; i < leftKeys.length; i++) {`);
      codegen.js(`var key = leftKeys[i];`);
      codegen.if(`rightKeys.indexOf(key) === -1`, () => {
        codegen.js(`return false;`);
      });
      codegen.if(`!${fnName}(left[key], right[key])`, () => {
        codegen.js(`return false;`);
      });
      codegen.js(`}`);
      codegen.js(`return true;`);
  ...
  protected generateCompareLess(leftVar: string, rightVar: string): string {
    ...
    codegen.if(`typeof left === "number" && typeof right === "number"`, () => {
      codegen.js(`return left < right;`);
    });
    codegen.if(`typeof left === "string" && typeof right === "string"`, () => {
      codegen.js(`return left < right;`);
    });
    codegen.js(`return false;`);
    codegen.js(`};`);
    ...
    switch (expression.name) {
      ...
      case 'search':
        if (expression.args.length !== 2) {
          ...
          codegen.if(`Array.isArray(${strVar})`, () => {
            codegen.js(`${strVar} = ${strVar}.length === 1 ? ${strVar}[0] : undefined;`);
          });
  ...
  protected genRecursiveDescentSelector(
    ...
    const lengthVar = codegen.var(`${inputVar}.length`);
    codegen.js(`for (var ${iVar} = 0, ${inputNodeVar}, ${descendantsVar}; ${iVar} < ${lengthVar}; ${iVar}++) {`);
    codegen.js(`${inputNodeVar} = ${inputVar}[${iVar}];`);
    codegen.js(`${descendantsVar} = [];`);
    // Collect descendants
    this.genCollectDescendants(inputNodeVar, descendantsVar);
    // Apply wrapped selector to descendants
    this.generateSelector(selector.selector, descendantsVar, outputVar);
    codegen.js(`}`);
  }

  protected genCollectDescendants(nodeVar: string, outputVar: string): void {
    const codegen = this.codegen;
