...
const rightClass = rule({
  ...
  flex: '1 1',
  // `min-width: 0` lets this column shrink below its content's intrinsic
  // width inside the flex row, which is what enables the text to truncate
  // with an ellipsis instead of overflowing/pushing the icon.
  minW: 0,
  ov: 'hidden',
});
...
export const BadgeRight: React.FC<Props> = (props) => {
  const [t] = useT();
  ...
  return (
    <span
      ...
      style={{
        padding: `0 ${width * 0.2}px 0 ${width * (0.2 + (spacious ? 0.1 : 0))}px`,
