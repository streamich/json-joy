import {overlap, type Patch, PATCH_OP_TYPE, sfx} from './str';
...
export const cleanupPatch = (patch: Patch): void => {
  ...
  while (pointer < patch.length) {
    if (patch[pointer][0] === PATCH_OP_TYPE.EQL) {
      ...
      if (
        ...
        // Duplicate record.
        patch.splice(equalities[equalitiesLength - 1], 0, [PATCH_OP_TYPE.DEL, lastequality]);
  ...
  pointer = 1;
  while (pointer < patch.length) {
    if (patch[pointer - 1][0] === PATCH_OP_TYPE.DEL && patch[pointer][0] === PATCH_OP_TYPE.INS) {
      ...
      if (overlap_length1 >= overlap_length2) {
        if (overlap_length1 >= deletion.length / 2 || overlap_length1 >= insertion.length / 2) {
          ...
          patch.splice(pointer, 0, [PATCH_OP_TYPE.EQL, insertion.substring(0, overlap_length1)]);
        ...
        if (overlap_length2 >= deletion.length / 2 || overlap_length2 >= insertion.length / 2) {
          ...
          patch.splice(pointer, 0, [PATCH_OP_TYPE.EQL, deletion.substring(0, overlap_length2)]);
...
const cleanupSemanticLossless = (patch: Patch): void => {
  ...
  while (pointer < patch.length - 1) {
    if (patch[pointer - 1][0] === PATCH_OP_TYPE.EQL && patch[pointer + 1][0] === PATCH_OP_TYPE.EQL) {
