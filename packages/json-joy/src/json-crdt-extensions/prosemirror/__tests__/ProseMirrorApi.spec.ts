...
import {assertCanMergeTrain, assertCanMergeInto, assertEmptyMerge} from './setup';
...
describe('.mergePmNode()', () => {
  ...
  test('produces not changes when merging document into equivalent document', () => {
    assertEmptyMerge(fixtures.nestedInlinesWithAttributes);
    assertEmptyMerge(fixtures.realisticDoc);
    assertEmptyMerge(fixtures.fuzzer1);
  });
