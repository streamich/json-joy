...
describe('applyPatch', () => {
  ...
  describe('inline mark changes', () => {
    test('add emphasis', () => {
      assertPatch(
        doc(p('hello world')) as Node,
        doc(p('hello ', em('world'))) as Node,
      );
    });
    ...
    test('change mark type', () => {
      assertPatch(
        doc(p(em('hello'))) as Node,
        doc(p(strong('hello'))) as Node,
      );
    });
    ...
    test('nested marks', () => {
      assertPatch(
        doc(p('plain')) as Node,
        doc(p(em(strong('bold-italic')))) as Node,
      );
    });
  });
  ...
  describe('block-level structural changes', () => {
    ...
    test('swap two paragraphs', () => {
      assertPatch(
        doc(p('first'), p('second')) as Node,
        doc(p('second'), p('first')) as Node,
      );
    });

    test('change block type', () => {
      assertPatch(
        doc(p('heading')) as Node,
        doc(h1('heading')) as Node,
      );
    });

    test('paragraph to blockquote', () => {
      assertPatch(
        doc(p('quoted')) as Node,
        doc(blockquote(p('quoted'))) as Node,
      );
    });

    test('insert paragraph in the middle', () => {
      assertPatch(
        doc(p('one'), p('three')) as Node,
        doc(p('one'), p('two'), p('three')) as Node,
      );
    });
  ...
  describe('nested list changes', () => {
    test('add list item', () => {
      assertPatch(
        doc(ul(li(p('a')), li(p('b')))) as Node,
        doc(ul(li(p('a')), li(p('b')), li(p('c')))) as Node,
      );
    });
    ...
    test('remove list item', () => {
      assertPatch(
        doc(ul(li(p('a')), li(p('b')), li(p('c')))) as Node,
        doc(ul(li(p('a')), li(p('c')))) as Node,
      );
    });

    test('edit text in nested list', () => {
      assertPatch(
        doc(ul(li(p('first')), li(p('second')))) as Node,
        doc(ul(li(p('first')), li(p('SECOND')))) as Node,
      );
    });
  ...
  describe('heading level changes', () => {
    test('h1 to h2', () => {
      assertPatch(
        doc(h1('Title')) as Node,
        doc(h2('Title')) as Node,
      );
    });

    test('h2 to h3 with text change', () => {
      assertPatch(
        doc(h2('Old Title')) as Node,
        doc(h3('New Title')) as Node,
      );
    });
  });
  ...
  describe('whole document replacement', () => {
    test('completely different documents', () => {
      assertPatch(
        doc(p('old content')) as Node,
        doc(h1('New'), p('Different doc'), blockquote(p('Quoted'))) as Node,
      );
    });
